...
class AzureComputecls(AzureBaseCloudcls, BaseComputecls):
    ...
    def list_instances(self):
        ...
        for azure_instance in azure_instances:
            instance = AzureInstancecls(
                azure_instance, credentials=self._credentials)
            instances.append(instance)

        return instances

    def get_instance_by_id(self, instance_id): pass
    ...
    def create_instance(self, image_id=None, key_name=None, security_groups=None, instancetype_id="ExtraSmall", name=None, zone=None, subnet_id=None, private_ips=None, user_data=None, storage_name=None, username=None, password=None):
        ...
        result = self.__SMS.check_hosted_service_name_availability(
            service_name)
        if result is False:
            msg = "Hosted service with name:{} exists. please use different name".format(
                name)
            raise Exception(msg)
        ...
        if storage_account is None:
            ...
            response = self.__SMS.create_storage_account(
                storage_name, service_name, service_name, location=self._credentials['region_name'])
            import time
            for i in range(60):
        ...
        os_image_url = "http://{}.blob.core.windows.net/{}/{}".format(
            storage_name, service_name, target_blob_name)
        ...
        if image.os == 'Windows':
            os_config = WindowsConfigurationSet(computer_name=name,
            ...
            endpoint1 = ConfigurationSetInputEndpoint(name='rdp',
                                                      protocol='tcp', port='3389', local_port='3389',
                                                      load_balanced_endpoint_set_name=None,
                                                      enable_direct_server_return=False)
            endpoint2 = ConfigurationSetInputEndpoint(name='web', protocol='tcp',					port='80', local_port='80',
                                                      load_balanced_endpoint_set_name=None,
                                                      enable_direct_server_return=False)
        ...
        if image.os == 'Linux':
            ...
            os_config.domain_join = None
            endpoint1 = ConfigurationSetInputEndpoint(name='ssh', protocol='tcp',					 port='22', local_port='22',
                                                      load_balanced_endpoint_set_name=None,
                                                      enable_direct_server_return=False)
        ...
        os_hd = OSVirtualHardDisk(
            image_id, os_image_url, disk_label=target_blob_name)

        self.__SMS.create_hosted_service(service_name, service_name,
                                         location=self._credentials['region_name'])
        service_account = self.__SMS.get_hosted_service_properties(
            service_name)

        self.__SMS.create_virtual_machine_deployment(service_name=service_name,
                                                     deployment_name=service_name, deployment_slot='production',
                                                     label=service_name, role_name=service_name, system_config=os_config,
                                                     os_virtual_hard_disk=os_hd, role_size=instancetype_id,
                                                     network_config=endpoint_config)

        new_service = self.__SMS.get_hosted_service_properties(service_name)
        azure_instance = AzureInstancecls(
            new_service, credentials=self._credentials)
        return azure_instance

    def create_instances(self, count=1, image_id=None, key_name=None, security_groups=None,
                         instancetype_id='Small', name=None, zone=None, subnet_id=None, private_ips=None, user_data=None): pass

    # --- Key pair operations -----------
    def list_keypairs(self): pass

    def create_keypair(self, name=None): pass
    ...
    def list_instancetypes(self):
        ...
        for azure_instancetype in azure_instancetypes_dict:
            instancetype = AzureInstanceTypecls(
                azure_instancetypes_dict[azure_instancetype], credentials=self._credentials)
            instancetypes.append(instancetype)
        return instancetypes

    def get_matching_instancetype(self, cpus=1, memory=0.5, disk=2): pass

    # -- Security group operations -----------
    def list_security_groups(self): pass

    def get_security_group_by_id(self, sg_id): pass

    def create_security_group(self, name=None, description=None): pass
