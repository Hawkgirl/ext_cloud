...
from AWSSecurityGroupRule import AWSSecurityGroupRulecls
...
class AWSSecurityGroupcls(AWSBaseCloudcls, BaseSecurityGroupcls):
	...
	def __init__(self, *arg, **kwargs):
        	self.__aws_security_group = arg[0]
                ...
                super(AWSSecurityGroupcls, self).__init__(id=self.__aws_security_group.id, name=self.__aws_security_group.name, credentials=kwargs['credentials'])
	...
	def add_rule(self, ip_protocol='tcp', from_port=None, to_port=None, cidr_block='0.0.0.0/0'):
		self.__aws_security_group.authorize(ip_protocol=ip_protocol, from_port=from_port, to_port=to_port, cidr_ip=cidr_block)

	def delete_rule(self, ip_protocol='tcp', from_port=None, to_port=None, cidr_block=None):
		self.__aws_security_group.revoke(ip_protocol=ip_protocol, from_port=from_port, to_port=to_port, cidr_ip=cidr_block)
