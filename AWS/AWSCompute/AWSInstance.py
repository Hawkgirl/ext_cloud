...
class AWSInstancecls(AWSBaseCloudcls, BaseInstancecls):
	...
	@property
        ...
        def _EC2(self):
                if self._ec2 is None:
                        self._ec2 = ec2.connect_to_region(self._credentials['region_name'], aws_access_key_id=self._credentials['username'], aws_secret_access_key=self._credentials['password'])
                return self._ec2
	...
	def creation_time(self):
		import datetime
		dt=datetime.datetime.strptime(self.__aws_instance.launch_time, '%Y-%m-%dT%H:%M:%S.%fZ')
	...
	@property
	def cpus(self):
		instance_type = self._COMPUTE.get_instancetype_by_id(self.instance_type)
