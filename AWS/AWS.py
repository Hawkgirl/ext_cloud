...
class AWScls(AWSBaseCloudcls, Cloudcls):
	...
	def compute(self):
		if self.__compute is None:
			self.__compute = AWSComputecls(username=self._credentials['username'], password=self._credentials['password'], region_name= self._credentials['region_name'])
	...
	def networks(self): 
		if self.__networks is None:
			self.__networks = AWSNetworkscls(username=self._credentials['username'], password=self._credentials['password'], region_name= self._credentials['region_name'])
	...
	def images(self):
		if self.__images is None:
			self.__images = AWSImagescls(username=self._credentials['username'], password=self._credentials['password'], region_name= self._credentials['region_name'])
	...
	def volumes(self): 
		if self.__volumes is None:
                        self.__volumes = AWSVolumescls(username=self._credentials['username'], password=self._credentials['password'], region_name= self._credentials['region_name'])
	...
	def objectstore(self): 
		if self.__objectstore is None:
                        self.__objectstore = AWSObjectStorecls(username=self._credentials['username'], password=self._credentials['password'], region_name= self._credentials['region_name'])
	...
	def templates(self):
		if self.__templates is None:
			self.__templates = AWSTemplatescls(username=self._credentials['username'], password=self._credentials['password'], region_name= self._credentials['region_name']) 
