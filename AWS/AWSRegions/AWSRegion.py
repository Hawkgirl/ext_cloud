...
class AWSRegioncls(AWSBaseCloudcls, BaseRegioncls):
	...
	def __init__(self, *args, **kwargs):
		...
		aws_region = args[0]
		super(AWSRegioncls, self).__init__(id=aws_region.name, name=aws_region.name, credentials=kwargs['credentials'])

	def list_zones(self):
		from boto.ec2 import connect_to_region

		conn = connect_to_region(self.name, aws_access_key_id=self._credentials['username'], aws_secret_access_key=self._credentials['password'])
