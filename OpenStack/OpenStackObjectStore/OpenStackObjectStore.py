...
class OpenStackObjectStorecls(OpenStackBaseCloudcls, BaseObjectStorecls):
        ...
        def __SwiftClient(self):
                if self.__swiftclient is None:
                        self.__swiftclient = s3.connect_to_region(self._credentials['region_name'], aws_access_key_id=self._credentials['username'], aws_secret_access_key=self._credentials['password'],calling_format=OrdinaryCallingFormat())
                return self.__swiftclient
