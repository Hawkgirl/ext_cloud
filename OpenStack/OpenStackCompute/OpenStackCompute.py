...
class OpenStackComputecls(OpenStackBaseCloudcls, BaseComputecls):
    ...
    def create_instance(self, image_id=None, key_name=None, security_groups=None, security_group_ids=None, instancetype_id=None, name=None, subnet_id=None):
        ...
        openstack_instance = self._NovaClient.servers.create(
            name, image_id, instancetype_id, nics=nics, security_groups=security_groups, key_name=key_name)
        instance = OpenStackInstancecls(
            openstack_instance, credentials=self._credentials)
        return instance

    def create_instances(self, count=1, image_id=None, key_name=None,
                         security_groups=None, instancetype_id=None, names=None): pass
    ...
    def list_hypervisors(self):
        ...
        for openstack_hypervisor in openstack_hypervisors:
            hypervisor = OpenStackHypervisorcls(
                openstack_hypervisor, credentials=self._credentials)
    ...
    def list_keypairs(self):
        ...
        for openstack_keypair in openstack_keypairs:
            keypair = OpenStackKeypaircls(
                openstack_keypair, credentials=self._credentials)
    ...
    def list_security_groups(self):
        openstack_security_groups = self._NovaClient.security_groups.list(search_opts={
                                                                          'all_tenants': 1})
        security_groups = []
        for openstack_security_group in openstack_security_groups:
            security_group = OpenStackSecurityGroupcls(
                openstack_security_group, credentials=self._credentials)
    ...
    def list_instancetypes(self):
        ...
        for openstack_instancetype in openstack_instancetypes:
            instancetype = OpenStackInstanceTypecls(
                openstack_instancetype, credentials=self._credentials)
            instancetypes.append(instancetype)

        return instancetypes

    def create_instancetype(self, name=None, ram=100, vcpus=1, disk=5):
        openstack_instancetype = self._NovaClient.flavors.create(
