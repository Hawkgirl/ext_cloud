...
class OpenStackComputecls(OpenStackBaseCloudcls, BaseComputecls):
    ...
    def list_metrics(self):
        ...
        metrics.append(BaseMetricscls('openstack.instances.total', len(
            instances)))
        metrics.append(BaseMetricscls(
            'openstack.instances.running', group_by_state[
                'STATE.RUNNING'] if 'STATE.RUNNING' in group_by_state else 0))
        metrics.append(BaseMetricscls(
            'openstack.instances.stopped', group_by_state[
                'STATE.STOPPED'] if 'STATE.STOPPED' in group_by_state else 0))
        metrics.append(BaseMetricscls(
            'openstack.instances.paused', group_by_state[
                'STATE.PAUSED'] if 'STATE.PAUSED' in group_by_state else 0))
        metrics.append(BaseMetricscls(
            'openstack.instances.error', group_by_state[
                'STATE.ERROR'] if 'STATE.ERROR' in group_by_state else 0))
        return metrics

    @property
    def Childrens(self):
        return self.list_hypervisors() + self.list_instances(
    ...
    def list_instances(self):
        ...
        for openstack_instance in openstack_instances:
            instance = OpenStackInstancecls(openstack_instance,
    ...
    def list_hypervisors(self):
        ...
        for openstack_hypervisor in openstack_hypervisors:
            hypervisor = OpenStackHypervisorcls(openstack_hypervisor,
                                                credentials=self._credentials)
    ...
    def list_keypairs(self):
        ...
        for openstack_keypair in openstack_keypairs:
            keypair = OpenStackKeypaircls(openstack_keypair,
                                          credentials=self._credentials)
    ...
    def list_security_groups(self):
        ...
        for openstack_security_group in openstack_security_groups:
            security_group = OpenStackSecurityGroupcls(
    ...
    def create_security_group(self,
                              ...
                              description=None,
                              network_id=None):
        openstack_security_group = self._NovaClient.security_groups.create(
            name, description)
        security_group = OpenStackSecurityGroupcls(
            openstack_security_group,
            credentials=self._credentials)
    ...
    def list_instancetypes(self):
        ...
        for openstack_instancetype in openstack_instancetypes:
            instancetype = OpenStackInstanceTypecls(
                openstack_instancetype,
                credentials=self._credentials)
    ...
    def create_instancetype(self, name=None, ram=100, vcpus=1, disk=5):
        openstack_instancetype = self._NovaClient.flavors.create(name=name,
                                                                 ram=ram,
                                                                 vcpus=vcpus,
                                                                 disk=disk)

        instancetype = OpenStackInstanceTypecls(openstack_instancetype,
                                                credentials=self._credentials)
        return instancetype
