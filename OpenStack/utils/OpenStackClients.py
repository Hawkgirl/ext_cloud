class OpenStackClientsCls:
    def get_neutron_client(self, credentials=None):
        ...
        if credentials.has_key('token'):
            ...
            neutronclient = NeutronClient.Client(token=credentials['token'],
    ...
    def get_glance_client(self, credentials):
        ...
        endpoint = keystoneclient.service_catalog.url_for(
            service_type='image',
            endpoint_type='publicURL')
        return GlanceClient.Client('2',
                                   endpoint=endpoint,
                                   token=token,
                                   cacert=credentials['cacert'])
