...
class OpenStackClientsCls:
    ...
    def get_neutron_client(self, credentials=None):
        ...
        if credentials.has_key('token'):
            keystone = KeystoneClient.Client(auth_url=credentials['auth_url'], token=credentials[
                                             'token'], tenant_name=credentials['tenant_name'], region_name=credentials['region_name'])
            endpoint = keystone.service_catalog.url_for(
                service_type='network', endpoint_type='publicURL')
            neutronclient = NeutronClient.Client(
                token=credentials['token'], endpoint_url=endpoint)
            return neutronclient

        neutronclient = NeutronClient.Client(username=credentials['username'], password=credentials['password'], tenant_name=credentials[
                                             'tenant_name'], auth_url=credentials['auth_url'], region_name=credentials['region_name'], ca_cert=credentials['cacert'])
    ...
    def get_nova_client(self, credentials):
        ...
        if credentials.has_key('token'):
            novaclient = NovaClient('2', credentials['username'], credentials['token'], credentials['tenant_name'], credentials[
                                    'auth_url'], 'compute', auth_token=credentials['token'], region_name=credentials['region_name'])

            return novaclient

        novaclient = NovaClient('2', credentials['username'], credentials['password'], credentials[
                                'tenant_name'], credentials['auth_url'], 'compute', region_name=credentials['region_name'])
